	...
	public void displayTable() {
	    ...
	    for (int i = 0; i < rows.size(); i++) {
	        ...
	        if (!ma_gv.equals(expectedData[i][0])) {
	            System.out.println("Lỗi kết quả: Mã GV không khớp. Expected: " + expectedData[i][0] + ", Actual: " + ma_gv);
	            isMatch = false;
	        }

	        if (!ten_gv.equals(expectedData[i][1])) {
	            System.out.println("Lỗi kết quả: Tên GV không khớp. Expected: " + expectedData[i][1] + ", Actual: " + ten_gv);
	            isMatch = false;
	        }

	        if (!loai_gv.equals(expectedData[i][2])) {
	            System.out.println("Lỗi kết quả: Loại GV không khớp. Expected: " + expectedData[i][2] + ", Actual: " + loai_gv);
	            isMatch = false;
	        }

	        if (!so_hp.equals(expectedData[i][3])) {
	            System.out.println("Lỗi kết quả: Số HP không khớp. Expected: " + expectedData[i][3] + ", Actual: " + so_hp);
	            isMatch = false;
	        }

	        if (!so_lop.equals(expectedData[i][4])) {
	            System.out.println("Lỗi kết quả: Số lớp không khớp. Expected: " + expectedData[i][4] + ", Actual: " + so_lop);
	            isMatch = false;
	        }

	        if (!so_gio_giang.equals(expectedData[i][5])) {
	            System.out.println("Lỗi kết quả: Số giờ giảng không khớp. Expected: " + expectedData[i][5] + ", Actual: " + so_gio_giang);
	            isMatch = false;
	        }
